...
const watchChanges = (dir, previousTimestamp, pendingReload) => {
    timestampForFilesInDirectory(dir).then(currentTimestamp => {
        ...
        files = Array.from(new Set([...Object.keys(previousTimestamp), ...Object.keys(currentTimestamp)]))
        // changedFiles = files.filter(file => previousTimestamp[file] !== currentTimestamp[file])
        hasChangedFiles = files.some(file => previousTimestamp[file] !== currentTimestamp[file])
